...
function AMR(G, liste_AMRs) # liste_AMRs est une liste de triplets (départ, arrivée, temps de départ)
    ...
    chemin1  = AStar2(G, liste_AMRs[1][1], liste_AMRs[1][2], liste_AMRs[1][3]) # On calcule le chemin optimal pour le 1er AMR
    if (chemin1 == []) # Si l'arrivée de AMR1 est inaccessible
    ...
    liste_chemins[1] = chemin1 # Pas de contrainte supplémentaire pour le premier AMR, on rajoute directement son chemin optimal
    ...
    for num_amr in 2:length(liste_AMRs) # On s'attaque aux AMRs suivants
        G_contr = copy(G) # On copie la grille si besoin de bloquer des cases en plus
        chemin = AStar2(G_contr, liste_AMRs[num_amr][1], liste_AMRs[num_amr][2], liste_AMRs[num_amr][3]) # On calcule le chemin optimal de l'AMR
        if (chemin == []) # Si l'arrivée est innateignable
            ...
            while (collision[1] != (0,0)) # On retente jusqu'à ne plus avoir de collision
                ...
                nouv_chemin = AStar2(G_contr, chemin[collision[2] - 1][1], liste_AMRs[num_amr][2], liste_AMRs[num_amr][3] + collision[2] - 2) # On part du point précédant la collision
            ...
            liste_chemins[num_amr] = chemin # Une fois que le bon chemin est trouvé, on le rajoute à la liste...
    ...
    for i in 1:length(liste_AMRs) # Et on montre les distances, coûts et temps des déplacements des AMRs
        println(string(" ", i, " : ", nb_pas(liste_chemins[i], G), " pas   coût=", length(liste_chemins[i])-1, "   (mission t=", liste_AMRs[i][3], "->t=", liste_chemins[i][end][2], ")"))
    end
